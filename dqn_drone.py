...
class CustomCNN(BaseFeaturesExtractor):
    def __init__(self, observation_space: gym.spaces.Dict, features_dim: int = 256):
        ...
        self.linear = nn.Sequential(
            nn.Linear(n_flatten + observation_space['position'].shape[0] + observation_space['velocity'].shape[0], features_dim),
            nn.ReLU(),
        )

    def forward(self, observations_space: th.Tensor):
        cnn_features = self.cnn(observations_space['image'].float() / 255.0)
        other_features = th.cat((observations_space['position'], observations_space['velocity']), dim=1)
